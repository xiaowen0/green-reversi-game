/**
 * Created by Kevin on 2020/11/26.
 */
...
var robot3 = {
    ...
    computeCellSuperiority : function (rowIndex, cellIndex, color)
    {
        ...
        var log = this.game.formatPositionCode(rowIndex, cellIndex) +
            ...
            ' 散度' + analysis.divergence;
        analysis.isEdge ? log += ' 边' : null;
        analysis.isCorner ? log += ' 角' : null;

        // approach center
        var result = 64;
        result += (6 - analysis.offsetToCenter) * 2;
        ...
        result += (8 - analysis.divergence) * 3;
    ...
    analyseCell : function (rowIndex, cellIndex, color)
    {
        ...
        var neighboringSituation = me.analyseCellNeighboringSituation(rowIndex, cellIndex, color);
        ...
        result.divergence = me.analyseEatingCellsDivergence(eatingCells, color);
        ...
        for (var i=0; i<eatingCells.length; i++)
        {
            ...
            result.divergence += eatingCellNeighboringSituation.empty;
     ...
     * @param   Array   cells
     */
    analyseEatingCellsDivergence : function(cells, color) {

        var divergence = 0;
        for (var i=0; i<cells.length; i++)
        {
            var situation = this.analyseCellNeighboringSituation(cells[i][0], cells[i][1], color);
            divergence += situation.empty;
        }

        // average divergence
        var average = divergence / cells.length;
